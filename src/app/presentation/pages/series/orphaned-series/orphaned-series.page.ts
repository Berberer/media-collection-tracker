...
@Component({
  selector: 'app-orphaned-series-page',
  templateUrl: './orphaned-series.page.html',
  styleUrl: './orphaned-series.page.css',
...
export class OrphanedSeriesPage implements OnInit, OnDestroy {
  ...
  readonly seriesViewMode = SeriesViewMode.ORPHANED;
  ...
  ngOnInit(): void {
    ...
    this.setUpGetOrphanedSeriesActionHandlers();
    this.setUpUpdateSeriesActionHandlers();
    this.setUpAddVolumeToOrphanedSeriesActionHandlers();
  ...
  private setUpGetOrphanedSeriesActionHandlers(): void {
    this.actions$
      .pipe(ofActionDispatched(Series.GetOrphaned), takeUntil(this.ngUnsubscribe))
      .subscribe(() => this.loadingSeries.set(true));

    this.actions$
      .pipe(ofActionCompleted(Series.GetOrphaned), takeUntil(this.ngUnsubscribe))
